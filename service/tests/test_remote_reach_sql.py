"""`scope_raw` / `remote_reach` against a real Postgres — the column wiring and the backfill.
...
`%s` template, and adding two columns moved every column after them. A shifted bind does not
necessarily fail: it writes one text column into another and both sides type-check. The only
proof is to write a row and read the fields back by name.
...
CASES = [
    ...
    (FakePosting("jobicy-emea", location="EMEA", remote_signal=True), "region"),
    (FakePosting("remotive-worldwide", location="Worldwide", remote_signal=True), "anywhere"),
...
def db():
    ...
    store._POOL = None
    store.upsert_postings([ingest.build_row(p) for p, _ in CASES])
    try:
        yield
    finally:
...
def stored() -> dict[str, dict]:
    with store.cursor() as cur:
        cur.execute(
            "select posting_id, location, scope_raw, remote_reach, work_mode, remote_signal "
            "from postings where posting_id like %s", (PREFIX + "%",))
        return {r["posting_id"]: dict(r) for r in cur.fetchall()}


def test_the_upsert_writes_both_new_columns_to_the_right_places():
    """The positional-bind guard. `_UPSERT_SQL` gained two columns in the middle of its list.
...
@pytest.mark.parametrize("posting, expected", CASES,
                         ids=[p.posting_id for p, _ in CASES])
def test_ingest_stores_the_expected_reach(posting, expected):
    assert stored()[posting.posting_id]["remote_reach"] == expected

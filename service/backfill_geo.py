...
Also re-derives the three columns that resolution improves, for the same reason ingest does:
`country_code` (null for whole sources until now), `remote_signal` (now means *fully* remote —
hybrid is excluded, which is the point of the feature) and, downstream of those,
`region`/`eligibility`.
...
from (values %s) as v(posting_id, country_code, city, remote_signal, region, eligibility)
where p.posting_id = v.posting_id
"""
_TEMPLATE = "(%s, %s::varchar(2), %s::text, %s::boolean, %s::text, %s::text)"
    ...
    while True:
        with store.cursor() as cur:
            cur.execute(
                ...
                "       region, eligibility, description "
        ...
        for r in rows:
            ...
            remote = geo.is_fully_remote(r["location"], r["description"], r["remote_signal"])
            region = work_region(r["location"], country)
            elig = eligibility(
                region, f"{r['title'] or ''} {r['location'] or ''} {r['description'] or ''}")
            before = (r["country_code"], r["city"], r["remote_signal"],
                      r["region"], r["eligibility"])
            after = (country, city, remote, region, elig)
            if before != after:
                updates.append((r["posting_id"], country, city, remote, region, elig))
